...
userRouter.post('/register', isGuest(),
  body('username').trim().isLength({ min: 5 }).withMessage('Usernamee must be at least 5 characters long!'),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const registerResult = await register(username, email, password, repass);
      ...
      res.render("register", { data: { username, email, password, repass }, errors: parserError(err).errors });
    }
  }
);
